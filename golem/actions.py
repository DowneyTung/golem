...
def repeat_click_and_wait(element, times=10, wait_time=45):
    """Click on the same element for multiple time
    Parameters:
    element: the webelement you want to click
    times: how many  times you want to repeately click on the same element
    wait_time: put some wait time while click on the webelement
    """
    try:
        for i in range(times):
            click(element)
            wait(wait_time)
            execution.logger.info("Click on element : {0} repeatedly for the {1} time".format(str(element), (i+1)))
    except Exception as ex:
        execution.logger.debug("Getting the stacktrace for clicking on {0} as: {1}".format(str(element),ex))
...
def switch_iframe(id):
    """switch iframe by iframe id
    Parameters:
    id: the iframe id
    """
    browser.get_browser().switch_to.frame(id)


def switch_default_content():
    """switch back to the default content"""
    browser.get_browser().switch_to.default_content()
...
def random_with_N_digits(n):
    ...
    range_start = 10**(n-1)
    range_end = (10**n) -1
    return rand.randint(range_start, range_end)
...
def execute_javascript(javascript, js_type=None):
    """execute javascript at the document root level
    Parameters:
    javascript: the javascript for executing
    js_type: name for storing the output from executing the javascript
    """
    _run_wait_hook()
    step_message = 'Execute javascript {0} at document root level as {1}'.format(javascript, js_type)
    driver = browser.get_browser()
    js_output = driver.execute_script(javascript)
    print('The js_output for js_type {0} is {1}'.format(js_type, js_output))
    store(js_type, js_output)
    execution.logger.info(step_message)
    _add_step(step_message)
    _screenshot_on_step()  
    return js_output
...
def http_get(url, headers={}, params={}, method=None, verify_ssl_cert=True, auth_required=False, username=None, password=None):
    ...
    if auth_required:
        response = requests.get(url, headers=headers, params=params, verify=verify_ssl_cert, auth=(username, password))
    else:
        response = requests.get(url, headers=headers, params=params, verify=verify_ssl_cert)

    if response.status_code in [204]:
        res_json = {"info": "The status code is 204 with no content"}
    else:
        res_json = json.loads(response.text)


    store(method, res_json)


def http_post(url, headers={}, data={}, method=None,verify_ssl_cert=True):
    ...
    method(str): value for storing the output  
    verify_ssl_cert (optional, default is True) : value
    """
    step_message = 'Make POST request to {}'.format(url)
    execution.logger.info(step_message)
    _add_step('Make a POST request to {}'.format(url))
    response = requests.post(url, headers=headers, json=data, verify=verify_ssl_cert)
    if response.status_code in [204]:
        res_json = {"info": "The status code is 204 with no content"}
    else:
        res_json = json.loads(response.text)
...
def http_put(url, headers={}, data=None, method=None,verify_ssl_cert=True):
    ...
    verify_ssl_cert (optional, default is True) : value
    """
    step_message = 'Make PUT request to {}'.format(url)
    execution.logger.info(step_message)
    _add_step('Make a POST request to {}'.format(url))
    response = requests.put(url, headers=headers, data=data, verify=verify_ssl_cert)
    if response.status_code in [204]:
        res_json = {"info": "The status code is 204 with no content"}
    else:
        res_json = json.loads(response.text)
...
def http_delete(url, headers={}, data={}, method=None,verify_ssl_cert=True):
    ...
    verify_ssl_cert (optional, default is True) : value
    """
    step_message = 'Make DELETE request to {}'.format(url)
    execution.logger.info(step_message)
    _add_step('Make a POST request to {}'.format(url))
    response = requests.delete(url, headers=headers, json=data, verify=verify_ssl_cert)
    if response.status_code in [204]:
        res_json = {"info": "The status code is 204 with no content"}
    else:
        res_json = json.loads(response.text)
...
def mysql_execute(host=None, user=None, password=None, query=None, method=None, port='3306',database=None, connect_timeout=2000,buffered=True):
    """Connect to specific mysql database and execute sql query.
    Parameters:
    host: the host ip of the db
    user: user name for connect
    password: password for connect
    query: the query string for execution
    database
    method: The method we used to store the return result
    port: the port number of the db, default value is 3306
    database: the database name
    connect_timeout: the timeout for making the sql database connection, default value is 2000
    buffered: whether the db query result is buffered or not
    """
    step_message = 'Make sql query to host {0} with user {1}, and the execute query string is {2}'.format(host, user, query)
    execution.logger.info(step_message)
    _add_step(step_message)
    ...
    try:
        ...
        execution.logger.debug('Is the sql connection closed? {}'.format(close_connection))
        cnx.close()
...
def get_text_in_element(element):
    """Get the text/value of webelement.
    Parameters:
    element: element
    """
    _run_wait_hook()
    webelement = browser.get_browser().find(element)
    step_message = 'Get the text in webelement {0} is {1}'.format(webelement.name, webelement.text)
    execution.logger.info(step_message)
