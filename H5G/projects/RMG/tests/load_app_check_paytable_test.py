description = 'Try to navigate to the lobby and check the dropdown tab related to paytable for Thundering Buffalo game'
...
def test(data):
    ...
    click(partner_game_lobby.thundering_buffalo_icon)
    ...
    click_on_target_area_with_coordinates('paytable_icon', partner_game_lobby.base_element, execution.data['element_x_coordinate'], execution.data['element_y_coordinate'])
    wait(2)
    assert_true(verify_element_exist_by_text('FREE SPINS BONUS'))
    wait(2)
